...
namespace AiEnabled.Bots
{
  public abstract partial class BotBase
  {
    ...
    {
      get { return (_botInfo & BotInfoEnum.CanOpenDoors) > 0; }
      set
      {
        if (value)
        {
          _botInfo |= BotInfoEnum.CanOpenDoors;
        }
        else
        {
          _botInfo &= ~BotInfoEnum.CanOpenDoors;
    ...
    public bool AllowHelmetVisorChanges
    {
      get { return (_botInfo & BotInfoEnum.AllowVisorChanges) > 0; }
      set
      {
        if (value)
        {
          _botInfo |= BotInfoEnum.AllowVisorChanges;
        }
        else
        {
          _botInfo &= ~BotInfoEnum.AllowVisorChanges;
    ...
    public bool HelmetEnabled
    {
      ...
      set
      {
        if (value && AllowHelmetVisorChanges)
        {
    ...
    {
      Character = bot;
      SpawnPosition = bot.WorldAABB.Center;
      ...
      AllowHelmetVisorChanges = AiSession.Instance.ModSaveData.AllowHelmetVisorChanges;
    ...
    internal virtual bool Update()
    {
      ...
      if (_tickCount % 150 == 0)
      {
        ...
        var lowHealth = healthRatio < 0.8f;

        if (lowHealth)
        {
          if (statComp.HasAnyComsumableEffect() && MyAPIGateway.Session.SessionSettings.AutoHealing)
          {
          ...
          if (!statComp.HasAnyComsumableEffect())
          {
            var inv = Character.GetInventory() as MyInventory;
            var def = new MyDefinitionId(typeof(MyObjectBuilder_ConsumableItem), "Medkit");
            if (inv != null && inv.GetItemAmount(def) > 0)
            {
              AiSession.Instance.HealingHash.Add(Character.EntityId);
              PlaySoundServer("AiE_FirstAid", Character.EntityId);
              inv.ConsumeItem(def, 1, Character.EntityId);
            }
          }
        }
        ...
        if (BehaviorReady)
        {
          ...
          if (lowHealth && Owner != null && _lowHealthTimer > 1800 && !(this is ScavengerBot))
          {
            BehaviorReady = false;
            _lowHealthTimer = 0;
    ...
    internal virtual bool IsInRangeOfTarget()
    {
      ...
      return Target.Override.HasValue || Target.IsFriendly() || Target.GetDistanceSquared() < 650000;
    }
    ...
    {
      if (AiSession.MainThreadId != Environment.CurrentManagedThreadId)
      {
        AiSession.Instance.Scheduler.Schedule(this.EquipWeapon);
        return;
      }
    ...
    internal bool CheckIfCloseEnoughToAct(ref Vector3D targetPosition, ref Vector3D goToPosition, out bool shouldReturn, out bool removeOverride)
    {
      shouldReturn = false;
      removeOverride = true;
      ...
      if (targetPosition == overridePos || (FollowMode && goToPosition == overridePos))
      {
        ...
        if (_currentGraph.IsGridGraph)
        {
          ...
          if (seat != null && !onLadder)
          {
            ...
            {
              if (Vector3D.DistanceSquared(botPos, targetPosition) > 6)
              {
              ...
              _hitList.Clear();
              MyAPIGateway.Physics.CastRay(botPos, seat.WorldAABB.Center, _hitList, CollisionLayers.CharacterCollisionLayer);
              ...
              foreach (var hit in _hitList)
              {
                ...
                if (hitGrid != null)
                {
                  var localHit = gridGraph.WorldToLocal(hit.Position);
                  if (localHit == localTarget)
                    break;

                  var hitCube = hitGrid.GetCubeBlock(localHit) as IMySlimBlock;
                  if (hitCube == null)
                  {
                    localHit = gridGraph.WorldToLocal(hit.Position + hit.Normal * 0.1f);
                    hitCube = hitGrid.GetCubeBlock(localHit) as IMySlimBlock;
                  ...
                  if (hitCube != null && localHit != localTarget)
                  {
                    var cubeDef = hitCube.BlockDefinition as MyCubeBlockDefinition;
                    if (cubeDef.Id.SubtypeName.IndexOf("shower", StringComparison.OrdinalIgnoreCase) < 0)
                    {
                      return false;
                    }
                  }
      ...
      if (localBot != localTgt)
      {
        ...
        if (checkCube)
        {
          ...
          checkDistance = (float)Math.Ceiling(checkDistance * checkDistance);
        }
        else if (isFriendly)
        {
        ...
        {
          var dirVec = Character.WorldMatrix.GetClosestDirection(tgtPos - botPos);
    ...
    internal void CheckLineOfSight()
    {
      ...
      MyAPIGateway.Physics.CastRayParallel(ref muzzlePosition, ref tgtPosition, _hitListThread, CollisionLayers.CharacterCollisionLayer, RayBlockedCallback);
    }
      ...
      if (NeedsTransition)
      {
        ...
        if (transitionOk && nextGraphOK && !_nextGraph.IsGridGraph)
        {
          ...
          for (int i = 0; i < entList.Count; i++)
          {
            ...
            {
              if (thisGrid != null && grid.IsSameConstructAs(thisGrid))
                continue;
      ...
      else if (!CheckGraphValidity(targetPosition, ref force, out newGrid, out newGraphPosition, out intermediatePosition, out botInNewBox))
      {
        ...
        if (!force)
        {
          ...
          else if (_nextGraph != null)
          {
            ...
            if (!botInNewBox || !_currentGraph.IsInBufferZone(botPosition) || !_nextGraph.IsPositionValid(botPosition))
            {
              switchNow = false;
              var direction = _currentGraph.WorldMatrix.GetClosestDirection(_nextGraph.OBB.Center - _currentGraph.OBB.Center);
              ...
              if (tgtPoint == null || nextIsPrev)
              {
                ...
                if (_nextGraph.RootVoxel != null)
                {
                  ...
                  MyTuple<float, float> tuple;

                  using (_nextGraph.RootVoxel.Pin())
                    tuple = _nextGraph.RootVoxel.GetVoxelContentInBoundingBox_Fast(box, _nextGraph.WorldMatrix);
          ...
          else if (NeedsTransition)
          {
            ...
            {
              bool result = _currentGraph.TryGetNodeForPosition(intermediateNode, out _transitionPoint);
            }
          }
        }
    ...
    internal void SwitchGraph(bool keepCurrent = true)
    {
      AdjustPatrolPointsForNewGraph(_currentGraph, _nextGraph);

      NeedsTransition = false;
      _pathCollection?.OnGridBaseClosing();
      _previousGraph = keepCurrent ? _currentGraph : null;
    ...
    internal void UsePathfinder(Vector3D gotoPosition, Vector3D actualPosition)
    {
      try
      {
        if (_currentGraph == null || !_currentGraph.Ready)
        {
          ...
          {
            CheckGraphNeeded = false;
            _nextGraph = AiSession.Instance.GetVoxelGraph(BotInfo.CurrentBotPositionActual, WorldMatrix, true);
        ...
        #region debugOnly
        if (AiSession.Instance.DrawDebug && _currentGraph.IsValid)
        ...
        bool returnNow, removeOverride;
        if (CheckIfCloseEnoughToAct(ref actualPosition, ref gotoPosition, out returnNow, out removeOverride))
        {
          CleanPath();
          ...
          if (!returnNow)
          {
            ...
            if (checkCube)
            {
              ...
              if (slim != null)
              {
                if (slim.FatBlock != null)
                {
                  if (!(this is RepairBot) && slim.FatBlock is IMyCockpit)
                    checkDistance = 2f;
                  else
              ...
              else
              {
                ...
                if (cube != null)
                {
                  if (!(this is RepairBot) && cube is IMyCockpit)
                    checkDistance = 2f;
                  else
            ...
            else if (FollowMode && Target.Override.HasValue && _currentGraph.GetBlockAtPosition(Target.Override.Value)?.FatBlock is IMyCockpit)
            {
              checkDistance = 2f;
            }
    ...
    void CheckNode(ref float distanceToCheck)
    {
      ...
      if (counterOverTwo || _pathCollection.UpdateDistanceToNextNode())
      {
        ...
        if (counterOverTwo)
        {
          ...
          if (addTo && _pathCollection != null)
          {
            ...
            if (addTo)
            {
              if (_currentGraph.AddToObstacles(last, current, next, true))
                _pathCollection.TempObstacles[next] = new byte();

              byte num;
              if (_pathCollection.TempObstacles.TryGetValue(next, out num))
              {
                num++;

                if (num > 5)
                {
                  _pathCollection.TempObstacles.TryRemove(next, out num);

                  if (_currentGraph.AddToObstacles(last, current, next, false))
                    _pathCollection.Obstacles[next] = new byte();
                }
                else
                {
                  _pathCollection.TempObstacles[next] = num;
                }
              }
      ...
      else
      {
        ...
        if (flattenedLengthSquared < distanceToCheck && Math.Abs(relVector.Y) < distanceToCheck)
        {
          ...
          {
            var pNext = _pathCollection.PathToTarget.Peek();
            var localVector = pNext.Position - pNode.Position;
            var upDir = _currentGraph.WorldMatrix.GetClosestDirection(WorldMatrix.Up);
            var upVec = Base6Directions.GetIntVector(upDir);

            if (upVec.Dot(ref localVector) > 0)
              UseLadder = true;
            else
              _pathCollection.ClearNode();
          }
    ...
    void FindNewPath(Vector3D targetPosition)
    {
      ...
      if (!found && !_currentGraph.GetClosestValidNode(this, goal, out adjustedGoal, isSlimBlock: isSlimBlock))
      {
        ...
        if (goBack)
        {
          ...
          {
            if (!_currentGraph.IsPositionValid(exit))
            {
              if (!NeedsTransition && !CheckGraphNeeded)
                CheckGraphNeeded = true;
          ...
          if (_idlePathTimer > 300)
          {
            ...
            if (graphReady)
            {
              ...
              if (!_transitionIdle)
              {
                ...
                var startPosition = botPosition;

                if (MaxTravelDistance > 0)
                {
                  minDistance = Math.Min(10, MaxTravelDistance);
                  maxDistance = Math.Min(MaxTravelDistance, (int)_currentGraph.OBB.HalfExtent.AbsMax());
                  startPosition = SpawnPosition;
                }
                else
                {
                  minDistance = 10;
                  maxDistance = (int)_currentGraph.OBB.HalfExtent.AbsMax();
                }

                var pos = startPosition + direction * MyUtils.GetRandomInt(minDistance, maxDistance);
    ...
    {
      return Character?.Name ?? $"{BotType}.{BotIdentityId}";
    }

    public override int GetHashCode()
    {
      return BotIdentityId.GetHashCode();
    }

    public override bool Equals(object obj)
    {
      var botObj = obj as BotBase;

      if (botObj != null && Character != null)
        return Character.Name == botObj.Character?.Name;

      return BotIdentityId == botObj.BotIdentityId;
    }
  }
}