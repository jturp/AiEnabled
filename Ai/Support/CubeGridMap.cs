...
//    void InitBlock(IMySlimBlock block, ref Vector3I upVec, ref Direction upDir, ref float cellSize, ref float halfCellSize, MyCubeBlockDefinition cubeDef)
//    {
...
//                if (j == 1)
//                  newNode.SetNodeType(NodeType.Ground, this);
...
//              if (PointInsideVoxel(worldPositionAbove, RootVoxel))
...
//                // TODO: Is this needed?
//              }
//              else if (AiSession.Instance.CatwalkBlockDefinitions.Contains(cubeAboveDef))
//              {
//                addThis = Base6Directions.GetIntVector(cubeAbove.Orientation.Up).Dot(ref upVec) <= 0;
//                includeAbove = addThis;
//              }
//              else if (AiSession.Instance.FlatWindowDefinitions.ContainsItem(cubeAboveDef))
//              {
//                Direction sideWithPane;
//                if (cubeDef.Id.SubtypeName == "LargeWindowSquare")
//                  sideWithPane = cubeAbove.Orientation.Forward;
//                else
//                  sideWithPane = Base6Directions.GetOppositeDirection(cubeAbove.Orientation.Left);

//                addThis = Base6Directions.GetIntVector(sideWithPane).Dot(ref upVec) >= 0;
//                includeAbove = addThis;
//              }
//              else if (AiSession.Instance.ArmorPanelFullDefinitions.ContainsItem(cubeAboveDef)
//                || AiSession.Instance.ArmorPanelHalfDefinitions.ContainsItem(cubeAboveDef))
//              {
//                addThis = Base6Directions.GetIntVector(cubeAbove.Orientation.Left).Dot(ref upVec) <= 0;
//                includeAbove = addThis;
//              }
//              else if (cubeAbove.FatBlock is IMyTextPanel 
//                || (cubeAbove.FatBlock is IMyLightingBlock && cubeAboveDef.SubtypeName == "LargeLightPanel"))
//              {
//                addThis = Base6Directions.GetIntVector(cubeAbove.Orientation.Forward).Dot(ref upVec) >= 0;
//                includeAbove = addThis;
//              }
...
//                          node.Update(adjacentPos, Vector3.Zero, this, NodeType.Ground, 0, this.MainGrid, adjacentBlock);
...
//        var blockOr = block.Orientation;
//        var dotUp = Base6Directions.GetIntVector(blockOr.Up).Dot(ref upVec);
//        var dotFwd = Base6Directions.GetIntVector(blockOr.Forward).Dot(ref upVec);
...
//              {
//                var node = AiSession.Instance.NodePool.Get();
//                node.Update(adjacentPos, Vector3.Zero, this, NodeType.Ground, 0, MainGrid, adjacentBlock);
//                AddTileToMap(adjacentPos, node);
//                ExemptNodesSide.Add(adjacentPos);
//              }
//            }
...
//          foreach (var cellPosition in positionList)
//          {
//            Node node = AiSession.Instance.NodePool.Get();
...
//    {
//      List<CubeGridMap> addMaps = AiSession.Instance.GridMapListPool.Get();
...
//              var newNode = AiSession.Instance.NodePool.Get();
//              newNode.Update(localTile, node.Offset, this, nodeType, node.BlockedMask, node.Grid, node.Block);

//              //OpenTileDict[localTile] = newNode; /*new Node(localTile, node.Offset, this, nodeType, node.BlockedMask, node.Grid, node.Block);*/
//              AddTileToMap(localTile, newNode);
//            }
//          }
...
//      bool isCatwalk = !isPlatform && AiSession.Instance.CatwalkBlockDefinitions.Contains(cubeDef.Id);
...
//        bool checkAbove = !exclude && (airTight || allowSolar || isCylinder || aboveisScaffold || isAllowedConveyor || isCatwalk
...
//              continue;
//            }
//            else if (blockSubtype.IndexOf("corridor", StringComparison.OrdinalIgnoreCase) >= 0
//              || blockSubtype.IndexOf("truss", StringComparison.OrdinalIgnoreCase) >= 0
//              || blockSubtype.IndexOf("extendedwindow", StringComparison.OrdinalIgnoreCase) >= 0)
//            {
//              if (CheckBlockForInvalidDirection(thisBlock, dirVec))
//              {
//                node.SetBlocked(dirVec);
//                continue;
//              }
...
//              //// Signal Update Corridor blocks
//              //if (blockSubtype == "Corridor" || blockSubtype.EndsWith("Window") || blockSubtype.EndsWith("Light"))
//              //{
//              //  if (dir != blockFwd && oppositeDir != blockFwd)
//              //  {
//              //    node.SetBlocked(dirVec);
...
//              //  }
//              //}
//              //else if (blockSubtype.EndsWith("X"))
//              //{
//              //  // can go in all 4 directions
//              //}
//              //else if (blockSubtype.EndsWith("T"))
//              //{
...
//              //{
//              //  if (oppositeDir != blockFwd && dir != thisBlock.Orientation.Left)
//              //  {
//              //    node.SetBlocked(dirVec);
//              //    continue;
//              //  }
...
//            if (blockBelowThis.Orientation.Up == upDir)
//            {
//              if (dirVec == -Base6Directions.GetIntVector(blockBelowThis.Orientation.Forward))
//              {
//                // block this direction in case there is a flat block so pathing doesn't think it can double back mid-air
//                node.SetBlocked(dirVec);
//                continue;
//              }
//            }
//            else if (blockBelowThis.Orientation.Forward == upDir && dirVec == Base6Directions.GetIntVector(blockBelowThis.Orientation.Up))
//            {
//              // block this direction in case there is a flat block so pathing doesn't think it can double back mid-air
//              node.SetBlocked(dirVec);
//              continue;
...
//                else if (blockSubtype.EndsWith("Corner"))
//                {
//                  if (dir != blockFwd && oppositeDir != nextBlock.Orientation.Left)
//                  {
//                    node.SetBlocked(dirVec);
//                    continue;
//                  }
...
//    public bool CheckCatwalkForRails(IMySlimBlock block, Vector3I side)
//    {
//      var def = block.BlockDefinition.Id;
//      Direction[] railArray;
//      if (!AiSession.Instance.InvalidBlockDirectionInfo.TryGetValue(def, out railArray))
//      {
//        return false;
//      }
...
//    public bool CheckBlockForInvalidDirection(IMySlimBlock block, Vector3I side)
//    {
//      var def = block.BlockDefinition.Id;
//      Direction[] invalidArray;
//      if (!AiSession.Instance.InvalidBlockDirectionInfo.TryGetValue(def, out invalidArray))
//      {
//        return true;
//      }
...
//      var checkDir = Base6Directions.GetDirection(side);
//      return invalidArray.ContainsItem(checkDir);
//    }
